        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <input value={form.property_id} onChange={e=>setForm(prev=>({ ...prev, property_id: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="Property ID" required />
            </div>
            <div>
              <label className="block text-sm text-slate-700 mb-1">Title</label>
              <input value={form.title} onChange={e=>setForm(prev=>({ ...prev, title: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="e.g., Replaced tap" required />
            </div>
            <div>
              <label className="block text-sm text-slate-700 mb-1">Category</label>
              <select value={form.category} onChange={e=>setForm(prev=>({ ...prev, category: e.target.value }))} className="w-full rounded-md border-slate-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="md:col-span-2">
              ...
              <textarea value={form.description} onChange={e=>setForm(prev=>({ ...prev, description: e.target.value }))} className="w-full rounded-md border-slate-300" rows={3} placeholder="What did you do?" />
            </div>
            <div>
              <label className="block text-sm text-slate-700 mb-1">Cost</label>
              <input type="number" step="0.01" value={form.cost} onChange={e=>setForm(prev=>({ ...prev, cost: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="0.00" />
