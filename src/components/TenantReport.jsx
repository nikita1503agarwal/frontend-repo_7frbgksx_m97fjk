...
export default function TenantReport() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 lg:px-8 py-8 grid md:grid-cols-2 gap-6">
      <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-xl p-5">
        <h3 className="text-lg font-semibold text-slate-900 mb-2">Generate tenant link</h3>
        <p className="text-sm text-slate-600 mb-3">Enter a property ID to create a one-time token. Share the token with the tenant so they can report an issue.</p>
        <form onSubmit={generateLink} className="space-y-3">
          <input value={genPropertyId} onChange={e=>setGenPropertyId(e.target.value)} className="w-full rounded-md border-slate-300" placeholder="Property ID" required />
          <div className="flex items-center gap-2">
            ...
            {generatedToken && (
              <span className="text-xs text-slate-700 break-all">Token: <code className="bg-slate-100 px-1 py-0.5 rounded">{generatedToken}</code></span>
            )}
          </div>
        </form>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-xl p-5">
        ...
        <form onSubmit={submitIssue} className="space-y-3">
          <div>
            ...
            <input value={form.token} onChange={e=>setForm(prev=>({ ...prev, token: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="Paste your token" required />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-700 mb-1">Your name</label>
              <input value={form.tenant_name} onChange={e=>setForm(prev=>({ ...prev, tenant_name: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="Optional" />
            </div>
            <div>
              <label className="block text-sm text-slate-700 mb-1">Contact</label>
              <input value={form.tenant_contact} onChange={e=>setForm(prev=>({ ...prev, tenant_contact: e.target.value }))} className="w-full rounded-md border-slate-300" placeholder="Phone or email (optional)" />
            </div>
          </div>
          <div>
            <label className="block text-sm text-slate-700 mb-1">Describe the issue</label>
            <textarea value={form.description} onChange={e=>setForm(prev=>({ ...prev, description: e.target.value }))} className="w-full rounded-md border-slate-300" rows={3} placeholder="What’s wrong?" required />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
            <div>
              <label className="block text-sm text-slate-700 mb-1">Priority</label>
              <select value={form.priority} onChange={e=>setForm(prev=>({ ...prev, priority: e.target.value }))} className="w-full rounded-md border-slate-300">
            ...
            <div>
              ...
              <input type="file" accept="image/*" onChange={e=>setForm(prev=>({ ...prev, photo: e.target.files?.[0] || null }))} className="w-full text-sm" />
            </div>
          </div>
          ...
          {successId && (
            <p className="text-green-700 text-sm">Issue submitted. Reference: {successId}</p>
          )}
        </form>
      </div>
    </section>
